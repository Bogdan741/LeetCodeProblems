...
//
//     lefti is the x coordinate of the left edge of the ith building. righti
//     is the x coordinate of the right edge of the ith building. heighti is
...
public:
    vector<vector<int>> getSkyline(vector<vector<int>>& buildings) {
        ...
        while(current < n || !liveBuildings.empty()){
            x = liveBuildings.empty() ? buildings[current][0] : liveBuildings.top().second;
            if ( current >= n || buildings[current][0] > x){
                while(!liveBuildings.empty() && (liveBuildings.top().second <= x)) liveBuildings.pop();
            } else {
                x = buildings[current][0];
                while(current < n && buildings[current][0] == x){
                    liveBuildings.push({buildings[current][2], buildings[current][1]});
                    ++current;
                }
            }
            y = liveBuildings.empty() ? 0 : liveBuildings.top().first;
            if(res.empty() || (res.back()[1] != y))
                res.push_back({x,y});
        }
        return res;
    }
